...
#import "MIHMotorViewController.h"
#import "MIHMotorCell.h"
...
@interface MIHMotorViewController ()

@end

@implementation MIHMotorViewController
...
- (void)viewDidLoad
{
    ...
    _headerView.backgroundColor = [UIColor colorWithRed:237.0/255 green:222.0/255 blue:168.0/255 alpha:1.0];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MIHMotorCell *cell = (MIHMotorCell*) [tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
   
    cell.mainPicView.layer.cornerRadius = cell.mainPicView.frame.size.width/2;
    cell.mainPicView.layer.masksToBounds = YES;
    
    PFObject *thisObj = [motorFeed objectAtIndex:indexPath.row];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([segue.identifier isEqualToString:@"showMotorDetail"]){
        
        MIHExperienceDetailViewController* nvc = (MIHExperienceDetailViewController*)[segue destinationViewController];
        
        
        PFObject *thisobject = [motorFeed objectAtIndex:[self.myTableView indexPathForSelectedRow].row];
